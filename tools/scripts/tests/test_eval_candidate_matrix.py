...
class EvalCandidateMatrixTests(unittest.TestCase):
    def test_required_analyzer_stdout_metrics_are_parsed(self) -> None:
        ...
        parsed = matrix.parse_analyzer_stdout(stdout)
    ...
    def test_optional_move_rank_metrics_are_parsed_and_rendered(self) -> None:
        ...
        self.assertEqual(
            matrix.move_rank_cells(parsed),
            ["3", "2", "0", "3", "2", "1", "4", "17", "9"],
        )

    def test_optional_move_rank_metrics_accept_negative_gap_sums(self) -> None:
        stdout = (
            "eval vs exact: records_read=5 analyzed=5 skipped=0 sign_agreements=4 "
            "wrong_direction=1 high_confidence_wrong_direction=0 "
            "move_rank_eval_score_gap_sum=-17 "
        ...
        self.assertEqual(parsed.move_rank_eval_score_gap_sum, -17)
        self.assertEqual(parsed.move_rank_exact_score_gap_sum, -9)
        self.assertEqual(
            matrix.move_rank_cells(parsed)[-2:],
            ["-17", "-9"],
        )

    def test_missing_required_analyzer_metric_is_rejected(self) -> None:
        with self.assertRaisesRegex(
            ScriptError,
            "analyzer stdout missing required metric: analyzed",
        ):
            matrix.parse_analyzer_stdout(
    ...
    def test_missing_move_rank_metrics_render_as_na(self) -> None:
        self.assertEqual(
            matrix.move_rank_cells(None),
            ["n/a"] * len(matrix.MOVE_RANK_METRIC_KEYS),
        )
